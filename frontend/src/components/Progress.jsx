...
export default function Progress({ user, onNavigate, onLogout, darkMode = false }) {
  ...
  const studyTimeData = analytics?.studyTimeData || [];
  const topicsProgressData = analytics?.topicsProgressData || [];
  const weakAreasData = analytics?.weakAreas || [];
  const confusionHeatmap = [];

  const stats = [
    { icon: BookOpen, label: 'Topics Studied', value: analytics?.topicsStudied || 0, change: '', color: 'from-purple-500 to-violet-500' },
    { icon: Clock, label: 'Total Study Time', value: `${analytics?.totalStudyTime || 0}h`, change: '', color: 'from-violet-500 to-purple-500' },
    { icon: Target, label: 'Quiz Score Avg', value: `${analytics?.averageScore || 0}%`, change: '', color: 'from-purple-600 to-violet-600' },
    { icon: Award, label: 'Flashcards Mastered', value: analytics?.flashcardsMastered || 0, change: '', color: 'from-violet-600 to-purple-600' }
  ];
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Charts */}
            {!loading && studyTimeData.length === 0 && topicsProgressData.length === 0 && (
              <div className="text-center py-12 mb-8">
                <TrendingUp className={`w-16 h-16 mx-auto mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
                <h3 className={`text-xl mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>No analytics data yet</h3>
                <p className={`${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Start studying to see your progress and insights</p>
              </div>
            )}
            {!loading && (studyTimeData.length > 0 || topicsProgressData.length > 0) && (

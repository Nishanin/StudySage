  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const p = res?.data || res?.user || null;
      ...
      <div className='flex-1 flex flex-col'>
        ...
        <main className='flex-1 p-4 md:p-8 overflow-y-auto'>
          <div className='max-w-6xl mx-auto'>
            {/* Profile Header Card */}
            <div
              className={`${darkMode ? "bg-gradient-to-br from-gray-800 to-gray-750 border-gray-700" : "bg-gradient-to-br from-white to-purple-50 border-purple-100"} rounded-3xl shadow-xl border p-8 mb-8`}>
                ...
                <div className='flex items-start gap-6'>
                  ...
                  <div>
                    <div className='flex items-center gap-3 mb-2'>
                      <h1
                        className={`text-4xl ${darkMode ? "text-white" : "text-gray-900"}`}>
                        {profileLoading
                          ? "Loading..."
                          : profile?.name || profile?.email || "Not available"}
                      </h1>
                    </div>
                    <div
                      className={`flex items-center gap-2 mb-4 ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
                    ...
                    <div className='grid grid-cols-2 gap-4 mt-4'>
                      <div
                        className={`flex items-center gap-2 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
                        <Mail className='w-4 h-4' />
                        <span className='text-sm'>
                          {profile?.email ||
                            (profileLoading ? "Loading..." : "Not available")}
                        </span>
                      </div>
                      <div
                        className={`flex items-center gap-2 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
              ...
              {/* Bio */}
              <div
                className={`mt-6 pt-6 border-t ${darkMode ? "border-gray-700" : "border-purple-100"}`}>
                <h3
                  className={`text-sm mb-2 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
                  Bio
                </h3>
                <p className={darkMode ? "text-gray-300" : "text-gray-700"}>
                  {profile?.bio || "Click edit button to add bio"}
                </p>
              </div>
            </div>
            ...
            <div className='grid lg:grid-cols-2 gap-8'>
              {/* Achievements */}
              <div
                className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-purple-100"} rounded-2xl shadow-lg border p-6`}>
                <div className='flex items-center justify-between mb-6'>
                  ...
                  <div
                    className={`px-3 py-1 ${darkMode ? "bg-gray-700" : "bg-purple-100"} rounded-lg`}>
                    <span
                      className={`text-sm ${darkMode ? "text-purple-400" : "text-purple-700"}`}>
                      0 Unlocked
                    </span>
                  </div>
                </div>
                ...
                {achievements.length === 0 ? (
                  <div className='text-center py-12'>
                    ...
                    <p
                      ...
                      No achievements earned yet. Keep studying to unlock
                      badges!
                    </p>
                  </div>
                ) : (
                  <div className='grid grid-cols-3 gap-4'>
                    {achievements.map((achievement, index) => (
                      <div
                        ...
                        <div
                          className={`w-12 h-12 ${achievement.color} rounded-xl flex items-center justify-center mb-3 mx-auto ${
                            !achievement.earned && "grayscale opacity-50"
                          }`}>
                          <achievement.icon className='w-6 h-6 text-white' />
                        </div>
                        <h4
                          className={`text-sm text-center mb-1 ${darkMode ? "text-white" : "text-gray-900"}`}>
                          {achievement.title}
                        </h4>
                        <p
              ...
              <div
                className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-purple-100"} rounded-2xl shadow-lg border p-6`}>
                <h2
                  className={`text-2xl mb-6 ${darkMode ? "text-white" : "text-gray-900"}`}>
                  Recent Activity
                </h2>
                ...
                <div className='space-y-4'>
                  {recentActivity.map((activity, index) => (
                    <div
                      ...
                      <div
                        className={`flex items-center gap-2 text-xs ${darkMode ? "text-gray-500" : "text-gray-500"}`}>
            ...
            {/* Study Preferences */}
            <div
              className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-purple-100"} rounded-2xl shadow-lg border p-6 mt-8`}>
              <h2
                className={`text-2xl mb-6 ${darkMode ? "text-white" : "text-gray-900"}`}>
                Study Preferences
              </h2>
              ...
              <div className='grid md:grid-cols-3 gap-6'>
                <div>
                  <label
                    className={`text-sm mb-2 block ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
                    Preferred Study Time
                  </label>
                  <select
                    className={`w-full px-4 py-3 ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-purple-50 border-purple-200"} border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500`}>
